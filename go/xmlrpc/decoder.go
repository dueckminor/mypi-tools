...
func assingString(val reflect.Value, value string) error {
	switch val.Kind() {
	case reflect.String:
		val.SetString(value)
		return nil
	case reflect.Interface:
		if val.NumMethod() == 0 {
			val.Set(reflect.ValueOf(string(value)))
			return nil
		}
	}
	return TypeMismatchError(fmt.Sprintf("error: type mismatch error - can't decode %v to string", val.Kind()))
}
...
func checkType(val reflect.Value, kinds ...reflect.Kind) error {
	...
	for _, kind := range kinds {
		...
		if val.Kind() == reflect.Interface && val.NumMethod() == 0 {
			// this breaks the currently disabled unit tests!
			match = true
			break
		}
	}
